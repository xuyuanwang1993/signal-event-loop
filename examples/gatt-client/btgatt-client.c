...
//env_defined
uint64_t time_last;
uint32_t warnning_delay_ms=100;
uint32_t total_test_cnts=1000;
bool    not_change_channel=false;
...
static void check_timestamp()
{
    ...
    uint32_t diff=now-time_last;
    if(diff>=warnning_delay_ms)
    {
        fprintf(stderr,COLOR_YELLOW "check[%d-%d] warnning! %u>=%u\r\n" COLOR_OFF,send_seq,total_test_cnts,diff,warnning_delay_ms);
    }
    time_last=now;
}
static void process_test_read_data(bool success, uint8_t att_ecode, const uint8_t *value,
                                   uint16_t length, void *user_data);
static void process_test_write_data(bool success, uint8_t att_ecode, void *user_data);

static void test_read(const char *input,struct client *cli)
{
    uint16_t handle;
    char *endptr = NULL;
    ...
    handle = strtol(input, &endptr, 0);
    if (!endptr || *endptr != '\0' || !handle) {
        printf("Invalid value handle: %s\n", input[0]);
        return;
    }

    if (!bt_gatt_client_read_value(cli->gatt, handle, process_test_read_data,
                                   cli, NULL))
        printf("Failed to initiate read value procedure\n");
    time_last=get_timestamp();
}

static void test_write(const char *handle_input,uint8_t * data,uint32_t length,struct client *cli)
{
    uint16_t handle;
    char *endptr = NULL;
    if (!bt_gatt_client_is_ready(cli->gatt)) {
    ...
    handle = strtol(handle_input, &endptr, 0);
    if (!endptr || *endptr != '\0' || !handle) {
...
void process_test_write_data(bool success, uint8_t att_ecode, void *user_data)
{
    usleep(10*1000);
    if(send_seq%100==0)
    {
        PRLOG("test-----------%d------------%d\r\n",send_seq,total_test_cnts);
    }

    struct client *cli=(struct client *)user_data;
    check_timestamp();
    if(send_seq>=total_test_cnts)return;
    int seq=send_seq%5;
    if((seq==0)||(seq==1))return;
    ++send_seq;
    if (success) {
        ...
        if(seq==2)
        {
            PRLOG("Write channel successful\n");
            const char * input="0x0034";
            uint8_t cmd[2]={0x03,0x00};
            test_write(input,cmd,2,cli);
        }else if (seq==3)
        {
            PRLOG("Write close uband successful\n");
            const char * input="0x0034";
            uint8_t cmd[2]={0x03,0x01};
            test_write(input,cmd,2,cli);
        }else if (seq==4) {
            PRLOG("Write open uband successful\n");
            const char *input="0x0030";
            test_read(input,cli);
        }
...
void process_test_read_data(bool success, uint8_t att_ecode, const uint8_t *value,
                            uint16_t length, void *user_data)
{
    usleep(10*1000);
    if(send_seq%100==0)
    ...
    int seq=send_seq%5;
    ++send_seq;
    if(seq==0&&length!=16)return;
    if(seq==1&&length!=1)return;

    if(seq==0)
    {
        PRLOG("read channel\r\n");
        uint8_t channel=value[0];
        if(!not_change_channel)
        {
            if(last_channel>=1&&last_channel<=150)
            {
        ...
        if(last_channel>150)last_channel=1;
        //read battery
        const char * input="0x000e";
        test_read(input,cli);
    }else if(seq==1) {
        PRLOG("read battery %d\r\n",value[0]);
        if(send_seq<3)
        {
            PRLOG("battery----------%d\r\n",value[0]);
        }
        const char * input="0x0034";
        uint8_t cmd[2]={0x04,last_channel};
        test_write(input,cmd,2,cli);
    }
}
...
static struct client *client_create(int fd, uint16_t mtu)
{
    ...
    if (verbose) {
        bt_att_set_debug(cli->att, att_debug_cb, ( void *)"att: ", NULL);
...
static void read_cb(bool success, uint8_t att_ecode, const uint8_t *value,
                    uint16_t length, void *user_data)
{
    process_test_read_data(success,att_ecode,value,length,user_data);
    //	int i;
...
{
    process_test_write_data(success,att_ecode,user_data);
    //	if (success) {
    //		PRLOG("\nWrite successful\n");
    //	} else {
    //		PRLOG("\nWrite failed: %s (0x%02x)\n",
...
static void usage(void)
{
    ...
    printf("Options:\n"
           ...
           "\t-c, --channel <channel> \t\tthe channel to set\n"
           "\t-T, --Testcnts <Testcnts> \t\tmax test cnts\n"
...
int main(int argc, char *argv[])
{
    ...
    while ((opt = getopt_long(argc, argv, "+hvs:m:t:d:i:c:T:",
                              main_options, NULL)) != -1) {
        switch (opt) {
        case 'h':
            usage();
        ...
        case 'c': {
            ...
            if (arg > 150) {
                fprintf(stderr, "channel too large: %d\n", arg);
                return EXIT_FAILURE;
            }

             last_channel= (uint8_t)arg;
             not_change_channel=true;
            break;
        }
        case 'T': {
            int arg;
            ...
            arg = atoi(optarg);
            if (arg < 0) {
                fprintf(stderr, "Invalid Testcnts: %d\n", arg);
                return EXIT_FAILURE;
            }

             total_test_cnts= arg*5;
            break;
        }
        case 't':
