...
CJsonObject* CJsonObject::CreateInstance(const std::string& path)
{
    CJsonObject* ret = new CJsonObject();
...
bool CJsonObject::SaveToFile()
{
    ...
    do {
        ...
        auto print_string = ToFormattedString();
        if (print_string.empty()) {
            break;
        }
        auto fp = fopen(m_save_path.c_str(), "w+");
        if (fp) {
