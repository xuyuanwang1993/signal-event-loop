...
public:
    ...
    void register_handle(bool use_thread=true);
    ...
    static void release(AimyLogger*logger){delete logger;}
private:
    AimyLogger();
    ~AimyLogger();
    /*判断文件是否打开*/
...
#ifndef AIMY_NO_LOG
#define AIMY_LOG(logger,level,fmt,...) (logger)->log(level,__FILE__, __FUNCTION__,__LINE__, fmt, ##__VA_ARGS__)
#define AIMY_LOG_D(level,fmt,...) AIMY_LOG(&(aimy::AimyLogger::Instance()),level,fmt,##__VA_ARGS__)
#define AIMY_DEBUG(fmt,...) AIMY_LOG_D(aimy::LOG_DEBUG,fmt,##__VA_ARGS__)
#define AIMY_INFO(fmt,...) AIMY_LOG_D(aimy::LOG_INFO,fmt,##__VA_ARGS__)
#define AIMY_WARNNING(fmt,...)  AIMY_LOG_D(aimy::LOG_WARNNING,fmt,##__VA_ARGS__)
#define AIMY_ERROR(fmt,...) AIMY_LOG_D(aimy::LOG_ERROR,fmt,##__VA_ARGS__)
#define AIMY_FATALERROR(fmt,...)  AIMY_LOG_D(aimy::LOG_FATALERROR,fmt,##__VA_ARGS__)
#define AIMY_MARK(fmt,...)  AIMY_LOG_D(aimy::LOG_BREAK_POINT,fmt,##__VA_ARGS__)
#define AIMY_BACKTRACE(fmt,...) AIMY_LOG_D(aimy::LOG_BACKTRACE,fmt,##__VA_ARGS__)

#else
#define AIMY_LOG(logger,level,fmt,...)
#define AIMY_LOG_D(level,fmt,...)
