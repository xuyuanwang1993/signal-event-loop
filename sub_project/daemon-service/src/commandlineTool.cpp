...
#define LOCAL_SERVICE_NAME "/tmp/drm-logo-service.service"
...
//        }
        defaultServerInit();
        AIMY_WARNNING("init server success ->%s:%hu",m_host.c_str(),m_port);
        return;
    }while(0);
...
bool commandLineTestTool::clientTask()
{
 ...
 bool commandLineTestTool::clientTask2()
 {
     std::unique_lock<std::mutex>locker(m_rawDataMutex);
     if(m_clientDataList.empty())
     {
...
bool commandLineTestTool::serverTask2()
{
    uint32_t buf_len=32*1024;
    char recv_buf[buf_len];
