...
private:
    ...
    /**
     * @brief m_maxLogfileCnts log文件最大数目
     */
    static uint32_t m_maxLogfileCnts;
    /**
...
public:
    ...
    /**
     * @brief getSnapshot获取系统快照
     * @param item_name 若为空，返回所有
     * @return
     */
    std::string getSnapshot(const std::string &item_name);
...
private:
    ...
    std::string packetSnapshotString(const std::string &item_name);
    /**
     * @brief addStdoutRedirectTask 添加标准输出重定向任务
     */
    void addStdoutRedirectTask(DaemonSession *session,int read_fd);
    /**
     * @brief removeStdoutRedirectTask 移除标准输出重定向任务
     */
    void removeStdoutRedirectTask(int read_fd);
    void stdoutRedirectThreadTask();
    void wakeup();
    /**
     * @brief handleStdoutRedirectTask 处理会话的标准输出
     * 若读到的长度<=0 判断任务出现异常 返回false,从任务列表中移除
     */
    bool handleStdoutRedirectTask(int fd, DaemonSession *session);
private:
    ...
    //
    std::shared_ptr<std::thread> m_stdoutRedirectThread;
    std::mutex m_socksMapMutex;
    std::map<int,DaemonSession *>m_stdoutRedirectTasksMap;
    int m_wakeupPipe[2];
...
struct MonitorHandle
{
    ...
    std::string snapshotCache;
    std::string data_cache;
    static bool isSnapShotBegin(const char * data,uint32_t len);
    static bool isSnapShotEnd(const char * data,uint32_t len);
    void updateSnapShotInfo(const char *data,uint32_t len);
    ~MonitorHandle(){
...
struct DaemonSession
{
    ...
    std::string subType;//子类型 normal or  monitor
    ...
    std::string logFilePath;//日志输出目录
    int64_t logFileSizeKBytesLimit;//日志文件输出大小限制
    bool saveLog;//是否保存log
    //runtime param
    pid_t pid;//当前进程id
    ...
    //monitor
    AimyLogger *loggerHandle;
    MonitorHandle *monitorHandle;
    //set by loadConfig
    std::string configName;
    std::string configFilePath;
    //
    DaemonWorker *const worker;
    //启动程序，若程序已启动 不产生影响
    void start();
    ...
    //获取当前时间戳
    static int64_t getClocktime();
    //重置进程状态，在进程回收时调用
    void resetBootStatus();
    //将进程设置成运行状态
    void setRunningStatus(pid_t new_pid,int64_t boot_time);
    /**
     * @brief handleStdoutMessage 处理标准输出的信息
     */
    void handleStdoutMessage(char * buf,uint32_t buf_len);
    /**
     * @brief flushStdoutMessage flush all stdout cache
     */
    void flushStdoutMessage();
public:
    DaemonSession(DaemonWorker *_worker);
    ~DaemonSession();
};
