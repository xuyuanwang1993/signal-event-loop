...
#define DAEMON_SERVICE_VERSION "1.0.1"
...
#define DAEMON_SUB_TYPE_1 "normal"
#define DAEMON_SUB_TYPE_2 "monitor"
...
uint32_t Daemon::m_maxLogfileCnts=20;
    ...
    do{
        ...
        while(m_working.load())
        {
            ...
            if(ret<0)
            {
                ...
                char buf[4]={0};
                uint32_t data_len=ret.size();
                buf[0]=data_len>>24;
                buf[1]=(data_len>>16)&0xff;
                buf[2]=(data_len>>8)&0xff;
                buf[3]=data_len&0xff;
                sock.send(buf,4,source_path);
                for(uint32_t i=0;i<data_len;i+=1024)
                {
                    uint32_t send_len=i+1024<=data_len?1024:data_len-i;
                    sock.send(ret.c_str()+i,send_len,source_path);
                }
...
bool Daemon::dumpConfig(const std::string &path)
{
    ...
    do{
        ...
        if(!DaemonFileHelper::appendKeyValueTofile(ConfigItemString[CONFIG_LOG_MAX_FILE_CNTS],std::to_string(m_maxLogfileCnts),"specify maximum numbers of log files,0 means no limit",fp))
        {
            AIMY_ERROR("write %s failed[%s]",path.c_str(),strerror(errno));
            break;
        }
        if(!DaemonFileHelper::appendKeyValueTofile(ConfigItemString[CONFIG_LOG_TO_TERMINAL],std::to_string(m_logToTerminal?1:0),
                                                   "is set to 1 log will print to terminal otherwise not",fp))
        {
            AIMY_ERROR("write %s failed[%s]",path.c_str(),strerror(errno));
            break;
        }
        if(!DaemonFileHelper::appendKeyValueTofile(ConfigItemString[CONFIG_STATUS_PRINT_INTERVAL_SEC],std::to_string(m_statusPrintIntervalSec?1:0),
                                                   "status print second inerval,if it's below to 0,status will not be print",fp))
        {
            AIMY_ERROR("write %s failed[%s]",path.c_str(),strerror(errno));
            break;
        }
...
void Daemon::processInitLog(const std::string pro_name,const std::string&path)
{
    ...
    aimy::AimyLogger::Instance().set_max_log_file_cnts(m_maxLogfileCnts);
...
std::string Daemon::handleOption(const std::string &option1,const std::string &option2)
{
    ...
    else if (option1=="snap") {
        ret=m_daemonWorker->getSnapshot(option2);
    }
...
void Daemon::handleCommandline(int argc ,char *argv[])
{
    static const char * helpInfo=
            ...
            __TIME__ " " __DATE__ "\n"
                                  ...
                                  "snap[taskName]\tget the snapshot information\n"
                                  "\n---------config-info--------\n"
        ...
        {//run
            ...
            while(1)
            {
                ...
                if(recv_len>0)
                {
                    ...
                    recv_data+=std::string(buf.get(),recv_len);
                    if(recv_data.size()==4)
                    {
                        data_len=static_cast<uint8_t>(recv_data[0])<<24;
                        data_len|=static_cast<uint8_t>(recv_data[1])<<16;
                        data_len|=static_cast<uint8_t>(recv_data[2])<<8;
                        data_len|=static_cast<uint8_t>(recv_data[3]);
                    }
                }
                else {
                    break;
                ...
                if(recv_data.size()>=4+data_len)break;
            }
            if(recv_data.size()<4)fprintf(stderr,"unix socket recv error[%s]\n",strerror(errno));
            else {
                printf("%s\n",recv_data.c_str()+4);
            }
        }
        ...
        for(auto i:items_map)
        {
            ...
            if(iter!=config_map.end())
            {
                ...
                case CONFIG_STATUS_PRINT_INTERVAL_SEC:
                    ...
                    break;
                case CONFIG_LOG_MAX_FILE_CNTS:
                    if(!i.second.empty())m_maxLogfileCnts=std::stoul(i.second);
                    break;
                default:
...
DaemonWorker::DaemonWorker(const std::string&configPath):m_configPath(configPath),m_workThread(nullptr),m_bRunning(false),m_statusPrintIntervalSec(30),m_lastPrintSec(0)
  ,m_stdoutRedirectThread(nullptr)
{
    loadAllConfig();
    pipe2(m_wakeupPipe, O_NONBLOCK | O_CLOEXEC);
}
...
void DaemonWorker::start()
{
    ...
    if(m_bRunning.load())
    {
        ...
        m_stdoutRedirectThread.reset(new std::thread([this](){
            this->stdoutRedirectThreadTask();
        }));
    }
}
...
void DaemonWorker::stop()
{
    ...
    if(m_bRunning.load())
    {
        ...
        if(m_stdoutRedirectThread)
        {
            if(m_stdoutRedirectThread->joinable())m_stdoutRedirectThread->join();
            m_stdoutRedirectThread.reset();
        }
    }
...
std::string DaemonWorker::reloadTask(const std::string &TaskName)
{
    ...
    std::shared_ptr<DaemonSession>session;
    if(iter!=m_tasksMap.end())
    {
        AIMY_INFO("stop old task:%s",TaskName.c_str());
        iter->second->releaseProcess();
        session=iter->second;
    }
    else {
        session.reset(new DaemonSession(this));
    }
...
{
    std::shared_ptr<DaemonSession>session(new DaemonSession(this));
...
std::string DaemonWorker::getSnapshot(const std::string &item_name)
{
    std::lock_guard<std::mutex>locker(m_dataMutex);
    if(m_stdoutRedirectTasksMap.empty())
    {
        return "no available task!";
    }
    return packetSnapshotString(item_name);
}
...
std::string DaemonWorker::packetSnapshotString(const std::string & item_name)
{
    std::string ret=AimyLogger::get_local_time()+"\n";
    ret+="\n----------snapshot----------\n";
    auto append_func=[](std::string & ret,const std::string &item_name,DaemonSession *session)
    {
        if(item_name.empty()||session->monitorHandle->snapshotInfo.empty())return;
        ret+="##########<"+item_name+">##########\n";
        ret+=session->monitorHandle->snapshotInfo;
        ret+="----------<"+item_name+">----------\n\n";
    };
    if(item_name.empty())
    {
        for(auto i:m_tasksMap)
        {
            append_func(ret,i.first,i.second.get());
        }
    }
    else {
        auto iter=m_tasksMap.find(item_name);
        if(iter!=m_tasksMap.end())
        {
            append_func(ret,item_name,iter->second.get());
        }
    }
    return ret;
}

void DaemonWorker::addStdoutRedirectTask(DaemonSession *session, int read_fd)
{
    {
        std::lock_guard<std::mutex>locker(m_socksMapMutex);
        m_stdoutRedirectTasksMap.emplace(read_fd,session);
    }
    wakeup();
}

void DaemonWorker::removeStdoutRedirectTask(int read_fd)
{
    {
        std::lock_guard<std::mutex>locker(m_socksMapMutex);
        m_stdoutRedirectTasksMap.erase(read_fd);
    }
    wakeup();
}

void DaemonWorker::stdoutRedirectThreadTask()
{
    AIMY_INFO("start stdoutRedirectThreadTask");
    std::map<int,DaemonSession *> runningMap;
    int64_t time_out_ms=5000;
    while(m_bRunning)
    {
        runningMap.clear();
        {
            std::lock_guard<std::mutex>locker(m_socksMapMutex);
            runningMap=m_stdoutRedirectTasksMap;
            //std::copy(m_stdoutRedirectTasksMap.begin(),m_stdoutRedirectTasksMap.end(),runningMap);
        }
        fd_set read_set;
        FD_ZERO(&read_set);
        FD_SET(m_wakeupPipe[0],&read_set);
        int max_fd=m_wakeupPipe[0];
        for(auto i: runningMap)
        {
            max_fd=max_fd>i.first?max_fd:i.first;
        ...
        if(ret<0)continue;
            ...
            //cv.wait_for接口使用的是绝对时间戳，在时间变化之后会引起异常，此处额外唤醒一次，用与更新触发时间更新
            std::lock_guard<std::mutex>locker(m_dataMutex);
            m_cv.notify_one();
        }
        if(FD_ISSET(m_wakeupPipe[0],&read_set))
        {
            char recv_buf;
            while(::read(m_wakeupPipe[0],&recv_buf,1)>0);
        }
        std::set<int>eraseSet;
        for(auto i:runningMap)
        {
           if(FD_ISSET(i.first,&read_set))
           {
               auto ret=handleStdoutRedirectTask(i.first,i.second);
               if(!ret)eraseSet.insert(i.first);
           }
        }
        {
            std::lock_guard<std::mutex>locker(m_socksMapMutex);
            for(auto i:eraseSet)
            {
                m_stdoutRedirectTasksMap.erase(i);
            }
        }
    }
    AIMY_INFO("stop stdoutRedirectThreadTask");
}

void DaemonWorker::wakeup()
{
    if(m_wakeupPipe[1]>0)::write(m_wakeupPipe[1], "1", 1);
}

bool DaemonWorker::handleStdoutRedirectTask(int fd,DaemonSession *session)
{
    //maybe released
    if(session->monitorHandle->readFd!=fd)return false;
    char buf[4096]={0};
    uint32_t total_read_len=0;
    while(1)
    {
        memset(buf,0,4096);
        auto read_len=::read(fd,buf,4096);
        if(read_len>0)
        {
            total_read_len+=read_len;
            std::lock_guard<std::mutex>locker(m_dataMutex);
            session->handleStdoutMessage(buf,static_cast<uint32_t>(read_len));
        }
        else {
            break;
        }
    ...
    {
        std::lock_guard<std::mutex>locker(m_dataMutex);
        session->flushStdoutMessage();
    }
    return  total_read_len>0;
}
...
bool MonitorHandle::isSnapShotBegin(const char *data, uint32_t len)
{
    if(len<8||data[0]!='#')return false;
    uint32_t count=0;
    while(count<len&&data[count]=='#')++count;
    return count==8;
}

bool MonitorHandle::isSnapShotEnd(const char *data, uint32_t len)
{
    if(len<10||data[0]!='#')return false;
    uint32_t count=0;
    while(count<len&&data[count]=='#')++count;
    return count==10;
}

void MonitorHandle::updateSnapShotInfo(const char *data,uint32_t len)
{
    if(snapshotCache.empty())
    {
        if(isSnapShotBegin(data,len))
        {
            snapshotCache.clear();
            snapshotCache+=AimyLogger::get_local_time()+"\n";
        }
    }
    else {
        if(isSnapShotEnd(data,len))
        {
            std::swap(snapshotInfo,snapshotCache);
            snapshotCache.clear();
        }
        else {
            snapshotCache+=std::string(data,len)+"\n";
        }
    }
...
void DaemonSession::check()
{
    do{
        ...
        {//running
            ...
            auto time_diff=getClocktime()-lastBootTime;
            ...
            if(autoRestart)
            {
                ...
                status=DaemonSessionWaitStarted;
                nextBootTime=getClocktime()+restartIntervalSec;
            }
        }
    }while(0);
    if(status==DaemonSessionWaitStarted){
        //wait run
        auto now_time=getClocktime();
...
{//*.conf
    ...
    do{
        ...
        for(auto i:items_map)
        {
            ...
            if(iter!=config_map.end())
            {
                ...
                case CONFIG_SUB_TYPE:
                    subType=i.second;
                    if(subType!=DAEMON_SUB_TYPE_1&&subType!=DAEMON_SUB_TYPE_2)
                    {
                        subType=DAEMON_SUB_TYPE_1;
                    }
                    break;
                case CONFIG_SAVE_LOG:
                    if(!i.second.empty())saveLog=i.second=="true";
                    break;
                case CONFIG_LOG_FILE_SIZE_LIMIT_KB:
                    if(!i.second.empty())logFileSizeKBytesLimit=std::stoul(i.second);
                    break;
                case CONFIG_LOG_PATH:
                    logFilePath=i.second;
                    break;
                default:
                    AIMY_WARNNING("%s=%s not supported!",i.first.c_str(),i.second.c_str());
                    break;
        ...
        {
            status=DaemonSessionWaitStarted;
            nextBootTime=getClocktime()+startDelaySec;
        }
        if(saveLog&&!logFilePath.empty())
        {
            loggerHandle->set_log_path(logFilePath,configName);
            loggerHandle->set_max_log_file_cnts(10);
            loggerHandle->set_log_file_size(logFileSizeKBytesLimit*1024);
            loggerHandle->set_log_to_std(false);
            loggerHandle->register_handle(false);
        }
        AIMY_INFO("load app_config:%s  [%s] success!",path.c_str(),configName.c_str());
        ret=true;
    }while(0);
    return ret;
}
...
pid_t DaemonSession::exec()
{
    ...
    do{
        ...
        if(worker)pipe2(pipe_fd, O_NONBLOCK | O_CLOEXEC);
        ret=fork();
        if(ret<0)
        {
            ...
            if(type==DAEMON_SHELL_TYPE)
            {
                ...
                if(!work_path.empty())chdir(work_path.c_str());
                if(!pro_path.empty()){execvp(pro_path.c_str(),argv);
                }
            }
            _Exit(0);
        }
    ...
    {
        ::close(pipe_fd[1]);
        monitorHandle->readFd=pipe_fd[0];
        if(worker)worker->addStdoutRedirectTask(this,monitorHandle->readFd);
    }
    return ret;
}
...
int64_t DaemonSession::getClocktime()
{
    struct sysinfo info;
    sysinfo(&info);
    return info.uptime;
...
void DaemonSession::resetBootStatus()
{
    ...
    pid=0;
    nextBootTime=0;
    if(monitorHandle->readFd>0)
    {
        if(worker)worker->removeStdoutRedirectTask(monitorHandle->readFd);
        ::close(monitorHandle->readFd);
        monitorHandle->readFd=-1;
    }
}
...
void DaemonSession::handleStdoutMessage(char * buf,uint32_t buf_len)
{
    monitorHandle->data_cache+=std::string(buf,buf_len);
    uint32_t start_pos=0;
    uint32_t offset=0;
    uint32_t max_len=monitorHandle->data_cache.size();
    const char *ptr=monitorHandle->data_cache.c_str();
    while(offset<max_len)
    {
        if(ptr[offset]=='\n')
        {
            if(offset>start_pos)
            {
                std::string message(ptr+start_pos,offset-start_pos);
                if(subType==DAEMON_SUB_TYPE_2)monitorHandle->updateSnapShotInfo(message.c_str(),message.size());
                AIMY_DEBUG("[%s] %s",configName.c_str(),message.c_str());
                if(loggerHandle){
                    AIMY_LOG(loggerHandle,aimy::LOG_DEBUG,"[%s] %s",configName.c_str(),message.c_str());
                }
            }
            ++offset;
            start_pos=offset;
    ...
    if(start_pos<max_len&&max_len-start_pos>2048)
    {
        std::string message(ptr+start_pos);
        if(subType==DAEMON_SUB_TYPE_2)monitorHandle->updateSnapShotInfo(message.c_str(),message.size());
        AIMY_DEBUG("[%s] %s",configName.c_str(),message.c_str());
        if(loggerHandle)AIMY_LOG(loggerHandle,aimy::LOG_DEBUG,"[%s] %s",configName.c_str(),message.c_str());
        monitorHandle->data_cache.clear();
    }
    else {
        monitorHandle->data_cache=monitorHandle->data_cache.substr(start_pos);
    }
}

void DaemonSession::flushStdoutMessage()
{
    if(monitorHandle->data_cache.size()>0)
    {
        std::string &message=monitorHandle->data_cache;
        if(subType==DAEMON_SUB_TYPE_2)monitorHandle->updateSnapShotInfo(message.c_str(),message.size());
        AIMY_DEBUG("[%s] %s",configName.c_str(),message.c_str());
        if(loggerHandle)AIMY_LOG(loggerHandle,aimy::LOG_DEBUG,"[%s] %s",configName.c_str(),message.c_str());
        monitorHandle->data_cache.clear();
...
bool DaemonSession::dumpConfig(const std::string&path)
{
    ...
    do{
        ...
        if(!DaemonFileHelper::appendKeyValueTofile(ConfigItemString[CONFIG_SUB_TYPE],subType,"sub type[normal/monitor]",fp))
        {
            AIMY_ERROR("write %s failed[%s]",path.c_str(),strerror(errno));
            break;
        }
        if(!DaemonFileHelper::appendKeyValueTofile(ConfigItemString[CONFIG_AUTO_START],autoStart?"true":"false",
                                                   "if it's true,it will be start after default loading",fp))
        {
            AIMY_ERROR("write %s failed[%s]",path.c_str(),strerror(errno));
            break;
        }
        if(!DaemonFileHelper::appendKeyValueTofile(ConfigItemString[CONFIG_AUTO_RESTART],autoRestart?"true":"false",
                                                   "if it's true ,it will be restart after the task is done",fp))
        {
            AIMY_ERROR("write %s failed[%s]",path.c_str(),strerror(errno));
            break;
        }
        if(!DaemonFileHelper::appendKeyValueTofile(ConfigItemString[CONFIG_SAVE_LOG],saveLog?"true":"false",
                                                   "if it's true,log will be saved",fp))
        {
            AIMY_ERROR("write %s failed[%s]",path.c_str(),strerror(errno));
            break;
        }
        if(!DaemonFileHelper::appendKeyValueTofile(ConfigItemString[CONFIG_LOG_FILE_SIZE_LIMIT_KB],std::to_string(logFileSizeKBytesLimit),"limit log_file size kb range[32k-2G],<=0 means no limit",fp))
        {
            AIMY_ERROR("write %s failed[%s]",path.c_str(),strerror(errno));
            break;
        }
        if(!DaemonFileHelper::appendKeyValueTofile(ConfigItemString[CONFIG_LOG_PATH],logFilePath,"specify logPath",fp))
        {
            AIMY_ERROR("write %s failed[%s]",path.c_str(),strerror(errno));
            break;
        }
...
std::string DaemonWorker::time_to_string(int64_t time)
{

    time_t tt=time-DaemonSession::getClocktime()+DaemonSession::getTimetamp();
...
std::pair<string, string> DaemonSession::getStatusString()
{
    ...
    sprintf(buf,"%-16.16s\t%-12.12s\t%-12d\t%-21.21s\t%-12d\t%-12d",configName.c_str(),status_str_func(status).c_str()
            ,pid,DaemonWorker::time_to_string(lastBootTime).c_str(),bootCnt,lastRebootCnt);
    return std::make_pair(std::string(head_str),std::string(buf));
}

DaemonSession::DaemonSession(DaemonWorker *_worker):sessionType(DAEMON_PROCESS_TYPE),subType(DAEMON_SUB_TYPE_1),execCmd(""),stopSignal(SIGTERM),killSignal(SIGKILL),autoStart(false),autoRestart(true)
  ,startDelaySec(0),restartIntervalSec(5),workPath(""),env(""),maxErrorRebootCnt(0),
    errorRebootThresholdSec(0),logFilePath(""),logFileSizeKBytesLimit(0),saveLog(false),pid(0),status(DaemonSessionExited)
  ,lastBootTime(0),nextBootTime(0),bootCnt(0),lastRebootCnt(0),errorBootCnt(0),loggerHandle(AimyLogger::create()),monitorHandle(new MonitorHandle)
  ,configName("default"),configFilePath(""),worker(_worker)
{
...
DaemonSession::~DaemonSession()
{
    ...
    if(loggerHandle){
        ...
        AimyLogger::release(loggerHandle);
    }
    if(monitorHandle){
        delete  monitorHandle;
